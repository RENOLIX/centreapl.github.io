...
export default async function Campaigns(){
  ...
  const [{data,error},{data:clients},{data:agentRows}]=await Promise.all([
    ...
    isManagement?supabase.from('clients').select('id,first_name,last_name,phone').order('created_at',{ascending:false}).limit(500):Promise.resolve({data:[]}),
  ...
  return <div className="space-y-6">
    ...
    {isManagement&&<CampaignManagement campaigns={campaigns.filter(c=>c.active).map(c=>({id:c.id,label:c.name}))} clients={clientOptions} agents={agentOptions}/>}
