...
type Row={id:string;title:string;scheduled_for:string;note:string;clients:{id:string;first_name:string;last_name:string;phone:string;email:string;city:string;metadata:Record<string,unknown>}|null}
type Assignment={clients:{id:string;first_name:string;last_name:string}|null}
export default async function Callbacks(){
  const role=await getCurrentRole();if(role!=='agent')redirect('/dashboard');const supabase=await createClient();const [{data},{data:assignments}]=await Promise.all([supabase.from('callbacks').select('id,title,scheduled_for,note,clients(id,first_name,last_name,phone,email,city,metadata)').eq('status','scheduled').order('scheduled_for',{ascending:true}).limit(2000),supabase.from('client_assignments').select('clients(id,first_name,last_name)').eq('status','active').limit(2000)])
  const callbacks=((data??[]) as unknown as Row[]).map(row=>({id:row.id,title:row.title||'Rappel',scheduledFor:row.scheduled_for,note:row.note,clientId:row.clients?.id||null,clientName:row.clients?`${row.clients.first_name} ${row.clients.last_name}`:'',client:row.clients?{phone:row.clients.phone,email:row.clients.email,city:row.clients.city,metadata:row.clients.metadata||{}}:null})) satisfies CalendarCallback[]
