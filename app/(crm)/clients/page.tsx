  ...
  return <div className="space-y-6">
    ...
    <section className="card overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        {data.map(client => <div key={client.id} className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-amber-100 font-black text-amber-800">{client.first_name[0]}{client.last_name[0]}</div><div><Link href={`/clients/${client.id}`} className="font-bold hover:text-amber-700">{client.first_name} {client.last_name}</Link><p className="text-xs text-slate-500">{client.city || 'Ville non renseignée'} · {client.phone}</p><p className="mt-1 text-[10px] font-bold uppercase text-emerald-700">{client.client_folders?.name || 'Sans dossier'}</p></div></div>
          <div className="flex flex-wrap items-center gap-2">{role === 'admin' && <ClientAdminActions client={client}/>}<a href={telHref(client.phone)} className="btn btn-primary text-sm"><Phone size={15}/>Appeler</a><Link href={`/clients/${client.id}`} aria-label="Ouvrir la fiche"><ChevronRight size={18} className="text-slate-400"/></Link></div>
        </div>)}
        {!data.length && <p className="p-8 text-center text-sm text-slate-500">{q ? 'Aucun client trouvé.' : 'Aucun client enregistré.'}</p>}
      </div>
    </section>
  </div>
}
