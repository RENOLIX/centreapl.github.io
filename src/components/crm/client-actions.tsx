...
export function ClientActions({ clientId, results, canCall }: { clientId: string; results: Result[]; canCall: boolean }) {
  ...
  async function saveCall(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const element=event.currentTarget; const form=new FormData(element); await submit(`/api/clients/${clientId}/calls`,{resultId:form.get('resultId'),summary:form.get('summary')},element) }
