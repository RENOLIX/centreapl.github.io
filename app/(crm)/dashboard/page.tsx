import { Clock3, DollarSign, Headphones, Megaphone, PhoneCall, RotateCcw, ShoppingCart, UsersRound } from 'lucide-react'
...
const formatDuration=(total:number)=>`${String(Math.floor(total/3600)).padStart(2,'0')}:${String(Math.floor(total%3600/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`
...
export default async function Dashboard() {
  ...
  try {
    const [clients, calls, successes, callbacks, sales, agents, todayAgentCalls, todayAgentCallbacks, campaignRows, activePauses, target] = await Promise.all([
      ...
      supabase.from('sales').select('*', { count: 'exact', head: true }).neq('status','cancelled').gte('sold_at',day.start).lt('sold_at',day.end),
  ...
  let ownPauseSeconds = 0
  let ownPauseStartedAt:string|null = null
  if (role === 'agent' && auth.user) {
    ...
    if (agent) {
      ...
      ownPauseSeconds = Math.floor((pauses ?? []).filter(row=>row.ended_at).reduce((total, row) => total + Math.max(0, new Date(row.ended_at!).getTime() - new Date(row.started_at).getTime()), 0) / 1000)
      ownPauseStartedAt = (pauses??[]).find(row=>!row.ended_at)?.started_at ?? null
    }
  }
  ...
  if(role==='admin'){
    ...
    tiles.push({label:'Appels terminés aujourd’hui',value:callsCount,Icon:PhoneCall,color:'#7447c8'},{label:'Rappels prévus aujourd’hui',value:callbackCount,Icon:Clock3,color:'#009688'},{label:'Ventes aujourd’hui',value:salesCount,Icon:ShoppingCart,color:'#e66b18',href:'/sales'})
  }
  if (isManagement) tiles.push({label:'Agents actuellement en pause',value:pausedAgents,Icon:Clock3,color:'#ef6c00',href:'/pauses'})
  if (role === 'agent') tiles.push({ label: 'Pause aujourd’hui (HH:MM:SS)', value: <PauseTotalTicker completedSeconds={ownPauseSeconds} startedAt={ownPauseStartedAt}/>, Icon: Clock3, color: '#ef6c00',href:'/pauses' })
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 xl:grid-cols-12">
      <div className="space-y-3 xl:col-span-9">
        ...
        {isManagement && <section className="card"><div className="panel-title">État du centre</div><div className={`grid gap-px bg-slate-200 ${role==='admin'?'sm:grid-cols-3':'sm:grid-cols-2'}`}><div className="flex items-center gap-4 bg-white p-5"><div className="grid h-12 w-12 place-items-center bg-[#ef5350] text-white"><Headphones size={25}/></div><div><p className="text-2xl font-light">{onlineAgents}</p><p className="text-[11px] text-slate-500">Agents en ligne maintenant</p></div></div><div className="flex items-center gap-4 bg-white p-5"><div className="grid h-12 w-12 place-items-center bg-[#009688] text-white"><UsersRound size={25}/></div><div><p className="text-2xl font-light">{performance.length}</p><p className="text-[11px] text-slate-500">Agents enregistrés</p></div></div>{role==='admin'&&<div className="flex items-center gap-4 bg-white p-5"><div className="grid h-12 w-12 place-items-center bg-[#ff7f24] text-white"><Megaphone size={25}/></div><div><p className="text-2xl font-light">{campaigns.length}</p><p className="text-[11px] text-slate-500">Campagnes actives</p></div></div>}</div></section>}
        {isManagement && <section className="card"><div className="panel-title">Activité des agents</div><div className="space-y-4 p-5">{performance.map(agent => { const maximum = Math.max(1, ...performance.map(item => item.calls.length)); const percent = Math.round(agent.calls.length / maximum * 100); return <div key={agent.id} className="grid grid-cols-[110px_1fr_40px] items-center gap-3 text-xs"><Link href={`/agents/${agent.id}`} className="truncate font-semibold text-slate-600 hover:text-emerald-700">{agent.users?.full_name || agent.code}</Link><div className="h-2 bg-slate-100"><div className="h-full bg-[#1db69a]" style={{ width: `${percent}%` }}/></div><span className="text-right text-slate-500">{agent.calls.length}</span></div>})}{!performance.length && <p className="py-8 text-center text-xs text-slate-400">Aucune activité agent enregistrée.</p>}</div></section>}
      </div>
